...
function Status({ data, loading }) {
  ...
  // Extracting balance from the provided data
  const availableBalance =
    parseFloat(data?.adminWalletDeets?.balance?.balance) || 0;

  return (
    <div className="w-full relative mt-10 mb-6 overflow-hidden h-[184px] rounded-xl">
      <Image src={card} alt="Card Background" className="cover" />
      <div className="relative w-full h-full px-10 flex items-center justify-between z-20">
          ...
          <>
            {visible ? (
              <>
                {loading ? (
                  <h2 className="text-[36px] font-semibold text-white">
                    ₦00<span className="text-gray-400">.00</span>
                  </h2>
                ) : (
                  <h2 className="text-[36px] font-semibold text-white">
                    ₦ {formatNumberWithCommas(availableBalance.toFixed(2))}
                  </h2>
                )}
              </>
            ) : (
              <div className="flex gap-0 mb-2">
                {[1, 2, 3, 4].map((dot, id) => (
                  <GoDotFill key={id} className="text-white" size={20} />
                ))}
              </div>
            )}
          </>
        ...
        >
          {visible ? (
            <Image src={eye} alt="Eye Icon" className="relative z-20" />
          ) : (
